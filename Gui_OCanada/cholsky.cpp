#include "cholsky.h"
#include "ui_cholsky.h"

Cholsky::Cholsky(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Cholsky)
{
    ui->setupUi(this);
...
void Cholsky::resolver_LU()
    {for (int k = 0; k < n; k++)
    {
        matrizL[k][k] = matrizU[k][k];
        double sum = 0;
        for (int p = 1; p < k - 1; p++)
        {
...
void Cholsky::gen_matrizAumentada() {
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
...
void Cholsky::definir_LU_simple()
    {
        double z, w;
        if (matrizA[0][0] == 0.0) {
            for (int i = 1; i < n; i++)
...
void Cholsky::on_bt_setAb_clicked()
{
    ingrese_matriz();
}

void Cholsky::on_solve_clicked()
{
    gen_matrizAumentada();
    definir_LU_simple();
    generar_U();
